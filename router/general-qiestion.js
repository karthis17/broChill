...
router.post('/share', async (req, res) => {
    try {
        const response = await general.findByIdAndUpdate(req.body.id, { $inc: { shares: 1 } })
...
router.post('/add-comment', auth, async (req, res) => {
    ...
    try {
        const response = await general.findByIdAndUpdate(req.body.id, { $push: { comments: { text: req.body.comment, user: req.user.id } } })
        res.status(200).json({
