...
router.post('/add-quizze', auth, adminRole, cpUpload, async (req, res) => {
    ...
    let { questions, results, description, language, resultImage, category, subCategory, isActive } = req.body;
    ...
    try {
        const qu = await contest.create({ questions, results, description, referenceImage: referencesImage, category, subCategory, language, user: req.user.id, isActive, resultImage });
...
router.get('/get-all', async (req, res) => {
    ...
    try {
        const fanquizzes = await contest.find({ language: lang, isActive: true }).populate({
            path: 'user',
...
router.get('/get/:id', async (req, res) => {
    ...
    try {
        const fanquizze = await contest.findById(req.params.id);
        res.json(fanquizze);
    } catch (error) {
        res.status(500).json(error.message);
    }
...
router.post('/get-result', cpUplad, async (req, res) => {
    ...
    try {
        ...
        const response = await contest.findById(quizze_id);
...
router.post('/:postId/like', auth, async (req, res) => {
    try {
        ...
        const post = await contest.findById(postId);
...
router.get('/share/:id', async (req, res) => {
    try {
        const response = await contest.findByIdAndUpdate(req.params.id, { $inc: { shares: 1 } }, { new: true });
        res.json(response);
    } catch (error) {
...
router.get('/view/:id', async (req, res) => {
    ...
    try {
        const response = await contest.findByIdAndUpdate(id, { $inc: { views: 1 } }, { new: true });
        res.json(response);
    } catch (error) {
        res.status(500).json({ message: 'Internal server error' });
    }
...
router.post('/add-comment/:id', auth, async (req, res) => {
    ...
    try {
        const response = await contest.findByIdAndUpdate(req.params.id, { $push: { comments: { text: req.body.comment, user: req.user.id } } }, { new: true }).populate({
            path: 'comments',
            populate: {
                path: 'user',
...
router.get('/publish/:postId', async (req, res) => {
    ...
    try {
        ...
        const result = await contest.findByIdAndUpdate(postId, { $set: { isActive: true } }, { new: true });
...
router.get('/draft/:postId', async (req, res) => {
    ...
    try {
        ...
        const result = await contest.findByIdAndUpdate(postId, { $set: { isActive: false } }, { new: true });
