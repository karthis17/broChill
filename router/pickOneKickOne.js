...
router.post('/:postId/like', auth, async (req, res) => {
    try {
        ...
        const post = await pickAndKick.findById(postId);
...
router.post('/add-comment/:id', auth, async (req, res) => {
    ...
    try {
        const response = await pickAndKick.findByIdAndUpdate(req.params.id, { $push: { comments: { text: req.body.comment, user: req.user.id } } }, { new: true }).populate({
            path: 'comments',
            populate: {
                path: 'user',
                select: ['-password', '-post']
